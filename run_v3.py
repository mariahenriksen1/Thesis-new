        ...
        for csv_path in tqdm(csv_paths, desc="Loading sequences"):
            ...
            if len(sequence_starts) > max_sequences_per_participant:
                sequence_starts = np.random.choice(
                    sequence_starts, 
                    size=max_sequences_per_participant, 
                    replace=False
                ).tolist()
...
with open(f"{results_dir}/v3_metrics.json", 'w') as f:
...
axes[0,1].set_title('V3: Accuracy')
axes[0,1].legend()
...
axes[1,1].set_title('V3: Metrics Summary')
axes[1,1].set_ylim([0, 1.0])
plt.setp(axes[1,1].xaxis.get_majorticklabels(), rotation=45, ha='right')

plt.tight_layout()
plt.savefig(f"{results_dir}/v3_comprehensive_results.png", dpi=300)
plt.close()
