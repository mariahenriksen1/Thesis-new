...
image_size = 16       # Small heatmap size
patch_size = 4        # 4x4 patches -> 4x4 grid
batch_size = 64
num_epochs = 5
frame_step = 100
device = "cpu"
model_save_path = "train_landmarks_v1.pth"
test_size = 0.2
...
def landmarks_to_heatmap(xs, ys, image_size=16, sigma=1.0):
    """Convert normalized landmarks to heatmap representation"""
...
class LandmarkDataset(Dataset):
    def __init__(self, data_list, image_size=16):
        self.data = data_list
...
def load_data(csv_folder, frame_step=100):
    """Load and sample landmark data from CSV files"""
    all_data = []
    ...
    for i, f in enumerate(files):
        ...
        # Sample every frame_step frames
...
print(f"Tiny Vision Transformer created with {total_params:,} parameters")
