...
for file in os.listdir(data_folder):
    if file.endswith("_CLNF_AUs_final.csv"):
        ...
        if df["Gender"].iloc[0] == 1:
            women_data.append(aligned_values)
        elif df["Gender"].iloc[0] == 0:
...
plt.plot(time_plot, women_plot.mean(axis=0), color='red', linestyle='--', linewidth=2, label='Depressed Women Mean')

# Men sequences
for series in men_plot:
    plt.plot(time_plot, series, color='skyblue', alpha=0.3)
plt.plot(time_plot, men_plot.mean(axis=0), color='blue', linestyle='--', linewidth=2, label='Depressed Men Mean')

plt.xlabel(f"Frame (every {downsample_step} frames)")
plt.ylabel(f"{au_column} Intensity")
plt.title(f"{au_column} Sequences for Depressed Men vs Women (Downsampled)")
plt.ylim(0, 5)
plt.legend()
plt.tight_layout()
plt.show()
