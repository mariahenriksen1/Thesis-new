...
# For gender: replicate the pattern (SMOTE only works on features, not metadata)
# We'll assign gender based on the original samples used by SMOTE
# Simple approach: use mode gender for synthetic samples or assign based on nearest original
gender_train_bal = np.zeros(len(y_train_bal), dtype=np.int64)
gender_train_bal[:len(gender_train)] = gender_train
# For synthetic samples, assign -1 (unknown) or use nearest neighbor logic
gender_train_bal[len(gender_train):] = -1

# Shuffle
perm = np.random.permutation(len(y_train_bal))
X_train_bal = X_train_bal[perm]
y_train_bal = y_train_bal[perm]
gender_train_bal = gender_train_bal[perm]

print(f"Balanced training set shape: {X_train_bal.shape}")
print(f"  Class 0: {np.sum(y_train_bal == 0)}")
print(f"  Class 1: {np.sum(y_train_bal == 1)}")
...
train_loader = DataLoader(
    AUDataset(X_train_bal, y_train_bal, gender_train_bal),
...
class AUTransformerWithGender(nn.Module):
    def __init__(self, num_features, num_classes=2, d_model=64, nhead=4, 
        ...
        # Gender embedding (0: female, 1: male, 2: unknown/synthetic)
...
# Save metrics
results_dir = "./au_v3_enhanced_results"
os.makedirs(results_dir, exist_ok=True)

metrics = {
    'model': 'AU-V3-Enhanced',
