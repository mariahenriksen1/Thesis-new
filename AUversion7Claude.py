...
print("="*70)
print("AU MODEL V7 - FIXED NORMALIZATION + ENHANCED TRAINING")
print("  + FIXED: Across-participant normalization (preserves differences)")
...
print("  + Focal Loss (better for imbalance)")
...
def extract_depression_features(df, au_columns):
    ...
    CRITICAL: Features are NOT normalized here - preserves absolute intensities
    """
    ...
    for row in features:
        # Original AUs (NOT normalized yet)
        au_features = list(row)
        
        # 1. Happiness co-activation: AU06 * AU12 (Duchenne smile)
        ...
        # 6. AU06-AU10 interaction (r=0.93 correlation)
...
for path in file_paths:
    ...
    # Extract depression-specific features (NOT NORMALIZED YET)
...
print(f"  Feature dimension: {X_train.shape[2]} (14 AUs + 6 depression features)")

# -----------------------------
# CRITICAL FIX: NORMALIZE ACROSS ALL PARTICIPANTS
# -----------------------------
print("\n🔧 APPLYING ACROSS-PARTICIPANT NORMALIZATION...")
print("   This preserves between-participant differences in AU intensities")

# Reshape to (all_frames, features) for normalization
...
X_test_flat = scaler.transform(X_test_flat)  # Use training statistics

# Reshape back to windows
X_train = X_train_flat.reshape(n_train_samples, seq_len, n_features)
X_test = X_test_flat.reshape(n_test_samples, seq_len, n_features)
...
train_temporal = temp_scaler.fit_transform(train_temporal)
test_temporal = temp_scaler.transform(test_temporal)
...
print(f"Learning rate scheduler: ReduceLROnPlateau (monitors AUC)")
...
# Save metrics
results_dir = "./au_v7_fixed_results"
os.makedirs(results_dir, exist_ok=True)

metrics = {
    'model': 'AU-V7-Fixed-Normalization',
    ...
    'features': '14 AUs + 6 depression-specific + 80 temporal (normalized across participants)',
    'normalization': 'FIXED: Across-participant normalization',
