# landmark_model_v2.py
# IMPROVED VERSION with COMPREHENSIVE EVALUATION
...
sequence_length = 60        # 2x longer
sequence_stride = 30
batch_size = 32
num_epochs = 10             # 2x more
test_size = 0.20
seed = 42

model_save_path = "landmark_v2.pth"
results_dir = "./landmark_v2_results"
os.makedirs(results_dir, exist_ok=True)

device = "mps" if torch.backends.mps.is_available() else ("cuda" if torch.cuda.is_available() else "cpu")
print(f"Device: {device}")
print("="*70)
print("MODEL VERSION: V2 - IMPROVED")
print("  Sequence Length: 60 frames (2x V1)")
print("  Epochs: 10 (2x V1)")
print("  Embed Dim: 128 (2x V1)")
print("  Layers: 3")
print("="*70)

random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)

# Dataset class (same as V1)
...
# MODEL SETUP
model = SequentialTransformer(landmark_dim=136, embed_dim=128, num_heads=8, num_layers=3, 
                              dropout=0.15, max_seq_length=sequence_length + 1).to(device)
...
print(f"\n{'='*70}")
print("COMPREHENSIVE EVALUATION - V2 RESULTS")
print(f"{'='*70}\n")
...
with open(f"{results_dir}/v2_metrics.json", 'w') as f:
    json.dump(metrics_v2, f, indent=2)

pd.DataFrame([metrics_v2]).to_csv(f"{results_dir}/v2_metrics.csv", index=False)
...
axes[0,0].set_title('V2: Confusion Matrix')
...
axes[0,1].set_title('V2: Accuracy')
axes[0,1].legend()
...
axes[1,0].set_title('V2: Validation AUC')
...
axes[1,1].set_title('V2: Metrics Summary')
axes[1,1].set_ylim([0, 1.0])
plt.setp(axes[1,1].xaxis.get_majorticklabels(), rotation=45, ha='right')

plt.tight_layout()
plt.savefig(f"{results_dir}/v2_comprehensive_results.png", dpi=300)
plt.close()
